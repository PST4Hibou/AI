...
from ..data.load_data import load_huggingface_dataset, create_data_loaders, load_test_datasets
...
class ModelComparator:
    ...
    def train_and_compare(self, epochs: int = 3, 
                         checkpoint_dir: str = "models/comparison") -> pd.DataFrame:
        """
        ...
        print(f"Training and comparing {len(self.model_names)} models for {epochs} epochs...")
    ...
    def plot_comparison_results(self, results_df: pd.DataFrame, 
        ...
        if 'total_parameters' in valid_results.columns and 'test_accuracy' in valid_results.columns:
            ax1 = axes[0, 0]
            scatter = ax1.scatter(valid_results['total_parameters'], 
        ...
        if 'training_time_s' in valid_results.columns and 'test_accuracy' in valid_results.columns:
            ax2 = axes[0, 1]
            ax2.scatter(valid_results['training_time_s'], 
                       valid_results['test_accuracy'],
        ...
        if 'samples_per_second' in valid_results.columns:
            ax3 = axes[1, 0]
            bars = ax3.bar(valid_results['model_name'], valid_results['samples_per_second'])
    ...
    def generate_comparison_report(self, results_df: pd.DataFrame, 
                                 output_path: str = "model_comparison_report.md"):
        """
        ...
        with open(output_path, 'w') as f:
            ...
            f.write("This report contains the results of comparing multiple model architectures ")
            f.write("for drone detection.\n\n")
            
            f.write("## Model Overview\n\n")
            f.write("| Model | Parameters | Size (MB) | Test Accuracy | Training Time (s) |\n")
            f.write("|-------|------------|-----------|---------------|------------------|\n")
            
            for _, row in results_df.iterrows():
                if 'error' not in row or pd.isna(row.get('error')):
                    params = f"{row.get('total_parameters', 0):,}" if 'total_parameters' in row else "N/A"
                    size = f"{row.get('model_size_mb', 0):.2f}" if 'model_size_mb' in row else "N/A"
                    acc = f"{row.get('test_accuracy', 0):.4f}" if 'test_accuracy' in row else "N/A"
                    time = f"{row.get('training_time_s', 0):.1f}" if 'training_time_s' in row else "N/A"
                    
                    f.write(f"| {row['model_name']} | {params} | {size} | {acc} | {time} |\n")
            ...
            if 'samples_per_second' in results_df.columns:
                fastest_inference = results_df.loc[results_df['samples_per_second'].idxmax()]
...
def quick_model_comparison(model_names: Optional[List[str]] = None, 
